...
// ref: github.com/siddontang/go/blob/master/time2
...
const (
	...
	tvn_mask uint64 = 63  //tvn_size - 1 -> 0011 1111
	tvr_mask uint64 = 255 //tvr_size -1  -> 1111 1111
)
...
func (w *Wheel) onTick() {
	...
	f := func(vec []*timer) {
		...
		for _, t := range vec {
			...
			if period := atomic.LoadUint64(&t.period); period > 0 {
				//t.expires = t.period + w.jiffies
				atomic.StoreUint64(&t.expires, period+w.jiffies)
				w.addTimer(t)
			}
		}
	}
...
func (w *Wheel) resetTimer(t *timer, when time.Duration, period time.Duration) {
	...
	atomic.StoreUint64(&t.expires, atomic.LoadUint64(&w.jiffies)+uint64(when/w.tick))
	atomic.StoreUint64(&t.period, uint64(period/w.tick))
